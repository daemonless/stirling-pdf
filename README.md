...
bastille create -O \
  ...
  --env PGID=1000 \
  --env TZ=UTC \
  --data-path /path/to/containers/stirling-pdf \
  stirling-pdf ghcr.io/daemonless/stirling-pdf:latest inherit
```
...
Save as `stirling-pdf-deploy.yaml`, then run `ansible-playbook stirling-pdf-deploy.yaml`.
